...
def calculate_distance(pickup, drop):
    """
    A simple placeholder function to calculate distance.
    In a real application, you would use a geocoding API for this.
    """
    # For demonstration, we'll return a hardcoded value based on range
    if (pickup == 'Sonipat' and drop == 'Delhi') or (pickup == 'Delhi' and drop == 'Sonipat'):
        return 80  # Example distance
...
def financial():
    ...
    # Define a fixed fuel price and a placeholder for fuel efficiency
    # NOTE: All hardcoded values are now converted to Decimal for consistent calculations
    fuel_price_per_liter = Decimal('96.0')  # Rs/LT
    fuel_efficiency_kmpl = Decimal('12.0')  # Assumed KMPL for the vehicle

    # 💰 Driver Salary Fix: Using a hardcoded value per trip
    driver_salary_per_trip = Decimal('800.0')  # Example: 800 Rs per trip

    # 💵 Toll/Misc Cost Fix: Using hardcoded values per trip
    toll_tax_per_trip = Decimal('100.0')  # Example: 100 Rs per trip
    misc_cost_per_trip = Decimal('50.0')  # Example: 50 Rs per trip

    # 🧾 Updated Query to get all required columns (mm.per_km_fuel_cost, mm.driver_salary, mm.toll_tax, mm.misc_cost and mm.criteria removed)
    query = """
    SELECT
        f.vehicle_id,
        f.driver_id,
    ...
    # Initialize summary with Decimal values
    summary = defaultdict(lambda: defaultdict(Decimal))
    routes = []

    # Calculate fuel cost per kilometer using the fixed values
    ...
    for row in rows:
        ...
        # 💰 Revenue = (Total Load * Transport Rate) + Loading + Unloading
        total_load = no_of_buckets * load_per_bucket
        revenue = (total_load * transport_rate) + loading_rate + unloading_rate

        # 💸 Cost = (Fuel Cost) + (Driver Salary) + (Toll/Misc)
        fuel_cost = total_km * cost_per_km_fuel

        # Using hardcoded costs for salary, toll, and miscellaneous
        ...
        trip = {
            ...
            'criteria': 'N/A',  # Since criteria is removed from DB, setting a default
...
def get_all_vehicle_numbers():
    """Retrieves all valid vehicle numbers from the fleet table for the dropdown."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT license_plate FROM fleet")
    rows = cursor.fetchall()
    conn.close()
    return [row[0] for row in rows] # Returns a list of strings

@app.route("/def", methods=["GET", "POST"])
def def_page():
    """
    Main route for displaying and adding indents.
    Handles form submissions for new indents and renders the main page.
    """
    ...
    if request.method == "POST":
        ...
        if vehicle_no not in valid_vehicles:
            ...
            try:
                # Insert the new indent into the database
                cursor.execute("""
                    INSERT INTO indents (
